...
/**
 * Step 5 — Certifications (dynamic entries)
 */
export default function CertificationsStep({ data, onChange }) {
  ...
  return (
    <div className="space-y-4">
      <h2 className="text-xl font-semibold text-gray-800 dark:text-gray-200">Certifications</h2>
      <p className="text-sm text-gray-500 dark:text-gray-400">List your professional certifications.</p>
        ...
        {data.map((cert, idx) => (
          ...
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <input placeholder="Issuer" value={cert.issuer} onChange={(e) => updateField(idx, 'issuer', e.target.value)} className="px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-900 text-sm text-gray-900 dark:text-gray-100" />
              <input placeholder="Year" value={cert.year} onChange={(e) => updateField(idx, 'year', e.target.value)} className="px-3 py-2 rounded-lg border border-gray-300 dark:border-gray-600 bg-gray-50 dark:bg-gray-900 text-sm text-gray-900 dark:text-gray-100" />
            </div>
          </motion.div>
        ))}
      </AnimatePresence>
